 ...
 */
const CONFIG = {
  // Ranges probed against a HCV 400 P2 on firmware 3.14, not taken from the
  // Home Assistant integration: the controller clamps the outdoor floor to
  // 12..17 itself, and accepts a setpoint from at least 10 up to 30. The floor
  // of 12 here is the app's, not the unit's — below it the bypass would open
  // on any mild day, cooling a house that is being heated.
  bypass_min_temp: { address: REG.BYPASS_MIN_TEMP, type: 'f32', min: 12, max: 17, firmware: 2.70 },
  bypass_max_temp: { address: REG.BYPASS_MAX_TEMP, type: 'f32', min: 12, max: 30, firmware: 2.70 },
  bypass_min_temp_summer: { address: REG.BYPASS_MIN_TEMP_SUMMER, type: 'f32', min: 12, max: 17, firmware: 3.08 },
  bypass_max_temp_summer: { address: REG.BYPASS_MAX_TEMP_SUMMER, type: 'f32', min: 12, max: 30, firmware: 3.08 },
...
/**
 * How hard to insist on an ACTIVE_MODE command before giving up. Bounded on
 * purpose — see setActiveFunction.
 */
const ACTIVE_MODE_ATTEMPTS = 3;
const ACTIVE_MODE_RETRY_MS = 4000;
...
/**
 * Fits a power curve to whatever operating points a commissioning report gives.
 *
 * Fan power is a constant electronics floor plus a cubic term:
 *
 *     P = idle + k · Q³
 *
 * which is linear in Q³, so a least-squares fit through the report's rows
 * recovers both constants. Two points are the minimum — with one, the floor
 * and the cubic cannot be separated, and guessing a split would put a number
 * on screen that no document backs.
 *
 * A Danish commissioning report tabulates minimum, standard and forced
 * ventilation, which is three points and a good fit. Elsewhere two will do.
 */
function fitPowerCurve(points) {
  const usable = (points || []).filter(
    (p) => p && p.flow > 0 && p.watts > 0 && Number.isFinite(p.flow) && Number.isFinite(p.watts),
  );
  if (usable.length < 2) return null;

  const xs = usable.map((p) => p.flow ** 3);
  ...
  const coefficient = covariance / variance;
  const idle = meanY - coefficient * meanX;

  // A negative cubic term would mean power falling as the fans speed up, which
  // means the numbers were mistyped rather than that the unit is remarkable.
  if (!Number.isFinite(coefficient) || !Number.isFinite(idle) || coefficient <= 0) return null;

  return { idle: Math.max(0, idle), coefficient };
}

/**
 * Power draw, from airflow and a curve fitted to the unit's own commissioning
 * figures. Nothing here is assumed about the model: an HCV 700 moving three
 * times the air draws its own numbers, and they come from its own report.
 *
 * The unit measures no power itself. A full sweep of the register map across a
 * 2.45x fan speed range found ten registers that move, all linear with speed —
 * rpm, its integer copies, and fan output percentage. A wattmeter would have
 * scaled by nearly fifteen and been impossible to miss.
 *
 * Accurate across the range the report covers. It stays well behaved outside
 * it — zero flow gives the standing draw, and the curve is monotonic — but
 * expect it to read low near maximum fan speed, where motor efficiency falls
 * off and the fitted cubic no longer holds.
 */
function estimatePower(flow, curve) {
  if (flow === null || flow === undefined || Number.isNaN(flow)) return null;
  if (!curve || !Number.isFinite(curve.idle) || !Number.isFinite(curve.coefficient)) return null;

  const watts = curve.idle + curve.coefficient * (flow ** 3);
  return watts < 0 ? 0 : Math.round(watts * 10) / 10;
}

/**
 * The share of the extract volume that comes back in as supply.
 *
 * A commissioned unit is deliberately unbalanced. Extract is set a few percent
 * above supply so the house sits at slight negative pressure, which draws
 * moisture out of the construction instead of pushing it into the walls where
 * it would condense inside the build-up. Danish practice is 4-8 %, and it is a
 * fixed property of the installation — but no register reports it and no
 * report column states it as a ratio.
 *
 * The exchanger gives it away. Energy across it balances, so
 *
 *   ṁ_supply · (supply − outdoor) = ṁ_extract · (extract − exhaust)
 *
 * and since both streams share a density, the volume ratio is simply the ratio
 * of the two temperature spans. On the reference installation that returns
 * 201 m³/h from a 216 m³/h extract — the report's own figure, to the litre.
 *
 * It needs a real temperature difference to divide by, so it returns null on a
 * mild day rather than amplifying sensor noise, and null while the bypass is
 * open, since then the exchanger is not in the loop for it to balance across.
 */
const MIN_EXCHANGER_SPAN = 8; // K — below this the ratio is mostly noise
const MIN_IMBALANCE = 0.85; // 15 % negative — past any sane commissioning
const MAX_IMBALANCE = 1.0; // supply above extract would pressurise the house

function supplyImbalance({
  supply, outdoor, extract, exhaust,
}) {
  if ([supply, outdoor, extract, exhaust].some((v) => v === null || v === undefined || Number.isNaN(v))) {
    return null;
  }

  const supplySpan = supply - outdoor;
  const extractSpan = extract - exhaust;
  if (supplySpan < MIN_EXCHANGER_SPAN || extractSpan < MIN_EXCHANGER_SPAN) return null;

  const ratio = extractSpan / supplySpan;

  // Condensation on the extract side releases latent heat the sensible balance
  // above does not model, which pushes the ratio up. Rather than quietly
  // absorbing that into the reading, an implausible answer is discarded.
  if (ratio < MIN_IMBALANCE || ratio > MAX_IMBALANCE) return null;
  return Math.round(ratio * 1000) / 1000;
}

/**
 * Thermal power the exchanger hands back, in kW.
 *
 ...
 *
 * Measured on the extract side, across the drop from extract to exhaust,
 * because that is the stream whose volume is known exactly: it is the figure
 * printed on the commissioning report. The supply side carries the same energy
 * by the balance above, but through a volume that has to be derived, so taking
 * it there would put an estimate inside an estimate.
 *
 ...
 * also transfers latent heat, and there this understates the recovery.
 */
const AIR_DENSITY = 1.2; // kg/m³ at room conditions
const AIR_HEAT_CAPACITY = 1.006; // kJ/(kg·K)

function recoveredHeat(flow, extract, exhaust) {
  if ([flow, extract, exhaust].some((v) => v === null || v === undefined || Number.isNaN(v))) {
    return null;
  }
  if (flow <= 0) return 0;

  const massFlow = (flow * AIR_DENSITY) / 3600;
  const kw = massFlow * AIR_HEAT_CAPACITY * (extract - exhaust);

  // A negative figure means the outgoing air is being warmed on its way out,
  // which is what free cooling looks like from this side — real, but not heat
  // handed back, so do not label it as such.
  return kw < 0 ? 0 : Math.round(kw * 100) / 100;
}

/**
 * Whether to lean on the fans to pull free cooling in, or to let go again.
 *
 * The unit decides on its own whether the bypass opens; what it will not do is
 * ventilate harder because there happens to be cold air outside worth having.
 * That is what this adds: while the house is above the setpoint and the air
 * outside is genuinely cooler, run the fans up and shift more of it.
 *
 * Only while the bypass is actually open. With it shut, the incoming air is
 * tempered back towards room temperature by the exchanger, so running harder
 * moves more air without cooling anything — noise and watts for nothing.
 *
 * And only in the modes where the user is asking for ordinary ventilation.
 * Away, night and fireplace were each chosen for a reason that outranks a
 * degree of comfort, and Dantherm's own summer mode stops the supply fan
 * entirely, so there is no incoming air to speed up.
 *
 * Engaging and releasing use different thresholds. Without that gap the fans
 * would step up and down every few minutes around the setpoint, which is worse
 * to live with than being one degree warm.
 */
const COOLABLE_MODES = ['automatic', 'manual', 'week_program'];
const COOL_ENGAGE_MARGIN = 0.3; // K above setpoint before it is worth starting
const COOL_USEFUL_GAP = 1.0; // K colder outside before the noise earns its keep
const COOL_RELEASE_GAP = 0.3; // K — narrower than the above, which is the hysteresis

function coolingDecision({
  indoor, outdoor, setpoint, bypassOpen, mode, boosting,
}) {
  // Losing the readings is not a reason to hold someone's fans at full speed.
  const blind = [indoor, outdoor, setpoint].some(
    (v) => v === null || v === undefined || Number.isNaN(v),
  );
  if (blind || !bypassOpen || !COOLABLE_MODES.includes(mode)) {
    return boosting ? 'release' : 'hold';
  }

  if (boosting) {
    const arrived = indoor <= setpoint;
    const spent = outdoor >= indoor - COOL_RELEASE_GAP;
    return arrived || spent ? 'release' : 'hold';
  }

  const tooWarm = indoor > setpoint + COOL_ENGAGE_MARGIN;
  const worthIt = outdoor < indoor - COOL_USEFUL_GAP;
  return tooWarm && worthIt ? 'boost' : 'hold';
}
...
class DanthermDevice {
  ...
  constructor({
    host, port = 502, unitId = 1, timeout = 3000, commandRetryMs = ACTIVE_MODE_RETRY_MS,
  }) {
    this.client = new ModbusTCP({ host, port, unitId, timeout });
    this.commandRetryMs = commandRetryMs;
    this.components = 0;
  ...
  /**
   * Turns one of the ACTIVE_MODE functions on or off.
   *
   * The register is a bitmask of what is currently running, and the documented
   * command pairs — start/end, select/deselect — turn out to be the same
   * instruction written twice. Both flip the bit, and the 0x8000 that appears
   * to mean "end" makes no difference at all. Verified on a HCV 400 P2 for
   * manual bypass: 0x0080 against a set bit cleared it, 0x8080 against a clear
   * bit set it, 0x8080 again cleared it. Taking the names at face value means a
   * Flow that turns a function off turns it on whenever it was already off —
   * which is worse than doing nothing, because it looks like it worked.
   *
   * So the bit is read first and the command sent only where it would change
   * something. That is right whether the register toggles or assigns, and the
   * distinction matters for a mapping worked out by experiment rather than
   * from a specification. Only manual bypass was probed directly; away and
   * night are assumed to behave the same, and this approach is correct either
   * way.
   *
   * Commands are also dropped outright while the bypass damper is travelling:
   * acknowledged, no error, no effect. Hence the read-back and a bounded
   * retry — a Flow card that blocks for a full damper swing is worse than one
   * that admits it could not do the job.
   */
  async setActiveFunction(bit, enabled, command) {
    for (let attempt = 0; attempt <= ACTIVE_MODE_ATTEMPTS; attempt++) {
      const active = await this.readUInt32(REG.ACTIVE_MODE);
      if (((active & bit) !== 0) === enabled) return;
      if (attempt === ACTIVE_MODE_ATTEMPTS) break;

      await this.writeUInt32(REG.ACTIVE_MODE, command);
      await new Promise((resolve) => { setTimeout(resolve, this.commandRetryMs); });
    }

    throw new Error('The unit kept ignoring the command, which it does while the bypass '
      + 'damper is moving. Try again once it has settled.');
  }

  async setManualBypass(enabled) {
    return this.setActiveFunction(
      ACTIVE_MODE.SELECT_MANUAL_BYPASS,
      enabled,
  ...
  /**
   * Closes the bypass damper. There is no command for this, and it took an
   * afternoon of writes to a live unit to be sure: the thresholds, the outdoor
   * floor, manual-bypass toggling and the damper register itself all leave an
   * open damper open. What closes it is standby. A stopped unit parks the
   * damper as its safe state, and it takes about a minute and a half to travel.
   *
   * Raise the ceiling before calling this or it opens straight back up.
   *
   * Restoring afterwards is verified rather than assumed. The first time this
   * ran, the restart command was quietly ignored and the house was left with no
   * ventilation at all — so the fans are watched until they actually spin.
   */
  async closeBypass({ onProgress = () => {}, timeoutMs = 180000 } = {}) {
    const before = await this.readState();
    if (before.bypassState === 'closed') return false;
    ...
    let state = before.bypassState;
    while (Date.now() < deadline && state !== 'closed') {
      await pause(8000);
      state = BYPASS_STATE[await this.readUInt32(REG.BYPASS_DAMPER)] || state;
      onProgress(state);
    }

    onProgress('restarting');
    if (before.mode === 'manual' && before.fanLevel > 0) {
      await this.setFanLevel(before.fanLevel);
    } else {
      await this.writeUInt32(REG.ACTIVE_MODE, ACTIVE_MODE.AUTOMATIC);
    }

    // Watch the fans come up, and fall back to a manual level if they do not.
    let running = false;
    for (let i = 0; i < 12 && !running; i++) {
      await pause(5000);
      const s = await this.readState();
      running = s.fanLevel > 0 && s.fan1Speed > 500;
    }
    if (!running) {
      onProgress('fallback');
      await this.setFanLevel(before.fanLevel || 3);
      for (let i = 0; i < 8 && !running; i++) {
        await pause(5000);
        const s = await this.readState();
        running = s.fan1Speed > 500;
      }
    }
    if (!running) throw new Error('The unit did not restart after closing the bypass');
