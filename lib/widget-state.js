...
function buildState(homey, deviceId) {
  ...
  return {
    ...
    recovered: bypassOpen
      ? null
      : recoveredHeat(airflow.extract, temperatures.extract, temperatures.exhaust),
    airflow,
    ...
    airflowNominal: device.getSetting('airflow_enabled')
      ? device.getSetting('report_flow_standard') || null
      : null,
...
/**
 * Takes an error from a widget's frontend and puts it in the app log.
 *
 * A widget runs inside a webview, so console.error there lands somewhere only
 * a developer with a USB cable and Safari Web Inspector will ever look. Routing
 * it back through the app means a user reporting trouble can send a diagnostic
 * report that actually contains the failure.
 */
function logError(homey, body) {
  const message = String(body?.message || 'unknown widget error').slice(0, 500);
  homey.app.error(`[widget${body?.widget ? `:${body.widget}` : ''}] ${message}`);
  return { ok: true };
}

module.exports = {
  buildState, setFanLevel, setMode, resolveDevice, logError,
};
